...
def findFromTableCnama(query):
    ...
    if len(rows) == 0 :
        print('could not found movie by this title!')
        return False
    return True

def findFromTableCnamaSeries(term):

    con = sqlite3.connect('fastmovie-online.db')
    cur = con.cursor()

    cur.execute("SELECT id FROM cnama_series WHERE `link` LIKE ? ;", (term,))
